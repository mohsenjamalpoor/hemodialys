...
const DrugAllergiesSection = React.memo(({
  ...
  const calculateStats = () => {
    ...
    const confirmedCount = safeAllergies.filter(item => item.confirmed).length;
    const withAlternatives = safeAllergies.filter(item => item.alternative).length;
    const commonDrugs = safeAllergies.filter(item => {
      const common = ['پنی‌سیلین', 'آسپرین', 'ایبوپروفن', 'سفالوسپورین', 'سولفا'];
      return common.some(drug => item.text.includes(drug));
    }).length;
    
    return { total, severe, confirmedCount, withAlternatives, commonDrugs };
  };
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-4 md:p-6 mb-6 transition-all duration-300 hover:shadow-xl">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-2 mt-1">
              ...
              {stats.severe > 0 && (
                <span className="px-2 py-1 bg-red-100 text-red-800 text-xs font-medium rounded-lg">
                  ⚠️ {stats.severe} مورد خطرناک
                </span>
              )}
              {stats.confirmedCount > 0 && (
                <span className="px-2 py-1 bg-green-50 text-green-700 text-xs font-medium rounded-lg">
                  ✅ {stats.confirmedCount} مورد تأیید شده
                </span>
              )}
              {stats.withAlternatives > 0 && (
                <span className="px-2 py-1 bg-blue-50 text-blue-700 text-xs font-medium rounded-lg">
                  💊 {stats.withAlternatives} مورد جایگزین دارد
                </span>
              )}
            </div>
          </div>
      ...
      {safeAllergies.length > 0 && (
        <div className="mt-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            ...
            <div className="bg-gradient-to-br from-red-50 to-pink-50 rounded-xl p-4 text-center border border-red-200">
              <p className="text-2xl font-bold text-red-600">{stats.severe}</p>
              <p className="text-xs text-red-700 mt-1">خطرناک</p>
            </div>
            <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-4 text-center border border-green-200">
              ...
              <p className="text-xs text-green-700 mt-1">تأیید شده</p>
            </div>
            <div className="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-xl p-4 text-center border border-blue-200">
              <p className="text-2xl font-bold text-blue-600">{stats.withAlternatives}</p>
              <p className="text-xs text-blue-700 mt-1">جایگزین دارد</p>
            </div>
            <div className="bg-gradient-to-br from-purple-50 to-violet-50 rounded-xl p-4 text-center border border-purple-200">
              <p className="text-2xl font-bold text-purple-600">{stats.commonDrugs}</p>
              <p className="text-xs text-purple-700 mt-1">شایع</p>
            </div>
          </div>
        </div>
      )}
      ...
      {/* خلاصه دسته‌بندی */}
      {safeAllergies.length > 0 && (
        <div className="mt-6 p-4 bg-gradient-to-r from-gray-50 to-blue-50 rounded-xl border border-gray-300">
          <h5 className="text-sm font-medium text-gray-700 mb-3">🔍 توزیع بر اساس گروه دارویی</h5>
          <div className="space-y-2">
            {['آنتی‌بیوتیک', 'مسکن', 'سولفا', 'انسولین', 'دیگر'].map((category) => {
              const count = safeAllergies.filter(item => 
                item.text.toLowerCase().includes(category.toLowerCase()) ||
                (category === 'دیگر' && !['آنتی‌بیوتیک', 'مسکن', 'سولفا', 'انسولین'].some(cat => 
                  item.text.toLowerCase().includes(cat.toLowerCase())
                ))
              ).length;
              
              if (count === 0) return null;
              
              const percentage = Math.round((count / safeAllergies.length) * 100);
